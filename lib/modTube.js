...
//	];
	...
	Tube.prototype.place = function (name, service, callback, messageId) {
		...
		var __this = this;
		messageId = (messageId || hexUuid.get());
		var child = new wractorJS({
			send: function(message, servable) {
				return __this.handle.call(__this, JSON.parse(message), servable); // Hack - Inject scope
			}
		});
		...
		child.send = child.handle.bind(child);
