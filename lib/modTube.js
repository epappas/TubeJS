...
var wrapperJS = require.resolve('./wrapper.js');
var wractorJS = require("./lib/wractor.js");
...
//	];
	var Wractor = (function (name) {
		...
		var __self = {
			...
			handle: function (message, servable) {
				...
				switch (message.type) {
					...
					case 8: // REP
						...
						break;
					case 9: // SRV
						this.serve(message.sender, message.value, servable);
						break;
					default:
						this.send(message.sender, message.value);
						break;
			...
			serve : function (type, servable) {
				...
				spawned.send(JSON.stringify(message), servable);
				return tube;
			},
	 ...
	 * @returns {*}
	 */
	Tube.prototype.handle = function (message, servable) {
		// if this is a responce
		...
		switch (message.type) {
			...
			case 8: // REP
				...
				break;
			case 9: // SRV
				this.handler.emit("serve", message, servable);
				break;
			default:
	...
	Tube.prototype.spawn = function (wractor, path, callback, messageId) {
		...
		var arr = process.execArgv.concat([wrapperJS], __args(arguments).slice(2) || []);
	 ...
	 * @returns {Function}
	 */
	Tube.prototype.place = function (name, service, callback, messageId) {
		if (typeof this.wractors[name] !== "undefined") {
			throw "Wractor " + name + " already exist.";
		}
		messageId = (messageId || hexUuid.get());
		var child = new wractorJS(process);

		child.name = name;
		service.apply(child, [child, child.name, ''].concat(Array.call(arguments)));
		...
		this.mailBox[messageId] = {
			...
			callback : function () {
				process.nextTick(function () {
					__this.emit("spawn", __this, name, __this.wractors[name]);
				});
				if (typeof callback === "function") {
					process.nextTick(callback.bind(__this, name, __this.wractors[name]));
				}
			}
		};

		child.send = child.emit.bind(this, "message", child);
		...
		this.wractors[name] = child;
		var __this = this;
		return Wractor.call(this, name);
	};
